...
public class Survey_CompositionController {
    ...
    public ResponseEntity<List<Survey_Composition>> getAllSurveyComposition() {
        try {
            List<Survey_Composition> surveyCompositions = new ArrayList<>();

            surveyCompositions.addAll(repository.findAll());

            if (surveyCompositions.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }

            return new ResponseEntity<>(surveyCompositions, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<Survey_Composition> addSurveyComposition(@RequestBody Survey_Composition survey_composition) {
        try {
